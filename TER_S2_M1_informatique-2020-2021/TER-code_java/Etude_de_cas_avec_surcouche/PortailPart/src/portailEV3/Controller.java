...
public class Controller{
	...
	private static MessageInt remoteControlCode;
	...
	private static MessageString vehiculeDemande;
	...
	public static void main(String[] args) throws InterruptedException, IOException, MessageException{
				...
				for(;;) {
					try {
						if(sensorDistance.obstacleDetect()) {//sensorVehicle.contact()
							...
							vehiculeDemande = (MessageString) wifiClient.receiveMessage();
							while(vehiculeDemande.getMessage() == "" || vehiculeDemande == null) {
								vehiculeDemande = (MessageString) wifiClient.receiveMessage();
							}
						}
		...
		while(app_alive){
			...
			if(vehiculeAutorisation.contains(vehiculeDemande.getMessage()) 
					&& (	close ||
